  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        const response = await fetch(`${baseUrl}/admin/technicians`, {
          method: "POST",
          headers: {
        ...
        if (data.success) {
          toast.success(
            data.emailSent
              ? "Technician added successfully and login credentials sent via email"
              : "Technician added successfully, but failed to send login credentials email"
          );
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Create a new technician profile</p>
        </div>
